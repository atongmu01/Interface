...
mode=cf.get("RECHARGE_FLAG","mode")
case_list=eval(cf.get("RECHARGE_FLAG","case_list"))

excel_data=excel_RW()
recharege_Data=excel_data.excel_Read(Excel_path,'recharge',mode,case_list)
mylogger=MyLogger("mylogger",Log_path)
@ddt
class Recharge_Request_test(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        print('开始测试')
        cf=configparser.ConfigParser()
    ...
    @data(*recharege_Data)
    @unpack
    def test_Recharge_Request(self,id,module,description,interface_address,mobilephone,amount,
        ...
        if method=='get':
            try:
                ...
                mylogger.mylog().info('用例编号为'+str(id)+'充值接口GET请求测试,手机号：'+str(mobilephone)+',当前余额为：'+str(LeaveAmount_old))
                excel_data.excel_Write(Excel_path,'recharge',int(id)+1,'actually_code',t.json()[code_name])
                self.assertEqual(t.json()[code_name],expect_code) #先检查接口返回数据是否通过
                excel_data.excel_Write(Excel_path, 'recharge', int(id) + 1, 'reason', t.json()["msg"])
                # 充值成功后，校验数据库数据
                if expect_code == '10001':
                    # 校验充值前后的余额
                    LeaveAmount_new=Mysql_Read().mysql_read(
                        "select m.LeaveAmount from member m where m.MobilePhone='" + str(mobilephone) + "'")[0]
                    mylogger.mylog().info('用例编号为'+str(id)+'充值接口GET请求测试,手机号：' + str(mobilephone) + ',充值后余额为：' + str(LeaveAmount_new))
                    if float(LeaveAmount_new)==float(LeaveAmount_old)+float(amount):
                        excel_data.excel_Write(Excel_path, 'recharge', int(id) + 1, 'db_check', 'pass')
                        excel_data.excel_Write(Excel_path, 'recharge', int(id) + 1, 'result', 'pass')
                        mylogger.mylog().info('GET请求测试结果正确，用例通过')
                    else:
                        excel_data.excel_Write(Excel_path, 'recharge', int(id) + 1, 'db_check', 'fail')
                        excel_data.excel_Write(Excel_path, 'recharge', int(id) + 1, 'result', 'fail')
